...
os.makedirs("hardware_results", exist_ok=True)
#### USER CHOOSE MODE
###mode- 0,1,2 
from qiskit_ionq import IonQProvider
...
def compute(qc, shots):
    ...
    provider = IonQProvider("dgtzbNFNI3oHnwK2VoGfDcZIjKw3Dwlt")
    backend = provider.get_backend("ionq_qpu.forte-enterprise-1")
...
def payoff(payoff_matrix,bitstring):
    payoff_1, payoff_2 = payoff_matrix[bitstring]
...
def expected_payoff(counts, payoff_matrix, shots):
    ...
    for bitstring, count in counts.items():
        ...
        payoff_1, payoff_2 = payoff_matrix[real]
